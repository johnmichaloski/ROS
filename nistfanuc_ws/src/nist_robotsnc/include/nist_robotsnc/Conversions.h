...
namespace Conversion {
    ...
    inline tf::Vector3 CreateVector(std::vector<double> ds) {
        assert(ds.size() > 2);
    ...
    inline tf::Pose CreateRPYPose(std::vector<double> ds) {
        ...
        tf::Pose p;
        p.setOrigin(tf::Vector3(ds[0], ds[1], ds[2]));
        p.setRotation(tf::Quaternion(ds[3], ds[4], ds[5]));
        return p;
    }
    ...
    inline tf::Pose CreatePose(tf::Quaternion q) {
        return tf::Pose(q, tf::Vector3(0., 0., 0.));
    }

    inline tf::Pose CreatePose(tf::Vector3 translation) {
        return tf::Pose(tf::Quaternion(0.0, 0.0, 0.0, 1.0), translation);
    }
    ...
    inline Eigen::Affine3d Rotation(double zangle) {
