...
namespace Nist {
    ...
    inline ValueType Convert(std::string str) {
    ...
    public:
        ...
        std::map<std::string, std::string> getmap(std::string section) {
            ...
            for (int i = 0; i < keys.size(); i++) {
                mapping[keys[i]] = GetSymbolValue(section + "." + keys[i]).c_str();
            }
            return mapping;
        }
    ...
    public:
        ...
        std::vector<std::string> GetTokens(std::string keyName, std::string delimiter) {
            std::string value = GetSymbolValue(keyName, "").c_str();
        ...
        std::vector<double> GetDblTokens(std::string keyName, std::string delimiter) {
            std::vector<std::string> sbasepose = GetTokens(keyName, delimiter);
            std::vector<double> ds(sbasepose.size(), 0.0);
            std::transform(sbasepose.begin(), sbasepose.end(), ds.begin(), [](const std::string & val) {
                return std::stod(val);
            });
            return ds;
        ...
        template <typename T>
        std::vector<T> GetTypedTokens(std::string keyName, std::string delimiter) {
            std::vector<std::string> sbasepose = GetTokens(keyName, delimiter);
            std::vector<T> ts;
            for (size_t i = 0; i < sbasepose.size(); i++) {
                T t;
                try {
                    t << sbasepose[i];
                } catch (...) {
        ...
        std::map<KeyType, ValueType> gettemplatemap(std::string section) {
            std::map<KeyType, ValueType> mapping;
            if (sections.find(section) == sections.end())
                return mapping;
            std::vector<std::string> keys = sections[section];
            for (int i = 0; i < keys.size(); i++) {
                try {
                    KeyType key;
                    key = Convert<KeyType>(keys[i]);
                    ValueType value = Convert<ValueType>(GetSymbolValue(section + "." + keys[i]).c_str());
                    mapping[key] = value;
                } catch (...) {
