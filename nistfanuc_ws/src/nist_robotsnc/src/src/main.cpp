...
//#define ROBOT  "motoman_sia20d"
#define ROBOT  "fanuc_lrmate200id"
...
std::string wspathsrc;

class CObstacleDemo {
    ros::Subscriber sub; /**<  clicked_point subscriber */
    CDetectCollision dc;
    JointStateUpdater &_updater;
public:

    CObstacleDemo(ros::NodeHandle &nh, JointStateUpdater &updater) : _updater(updater) {
        sub = nh.subscribe("clicked_point", 10, &CObstacleDemo::callback, this);
        // std::string robotname = RobotName(nh);
        std::string robotname = updater.Name();
        std::string urdf_file = StrFormat("%s%s_support/urdf/%s.urdf", wspathsrc.c_str(), robotname.c_str(), robotname.c_str());
        std::string srdf_file = StrFormat("%s%s_support/urdf/%s.srdf", wspathsrc.c_str(), robotname.c_str(), robotname.c_str());
        dc.Setup(nh, urdf_file, srdf_file);
        bool bCollision = dc.SanityCheck();
        std::cout << "SanityCheck: " << bCollision << "\n";

    }

    void callback(const geometry_msgs::PointStamped::ConstPtr& msg) {
        geometry_msgs::Point pt = msg->point;
        std::cout << StrFormat("geometry_msgs::PointStamped=%f:%f:%f\n", pt.x, pt.y, pt.z);
        std::map<std::string, double> jts = _updater.actual_jts();
        bool bCollision = dc.RobotCollisionCheck(jts);
        std::cout << "JointCheck: " << bCollision << "\n";
    }
};
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        wspathsrc = packagepath.substr(0, packagepath.find_last_of('/') + 1);
        ...
        ros::AsyncSpinner spinner(4); // thread count = 2?
        spinner.start();
        ...
        rvizMarker->Scale(0.5, 1.0, 0.05);
        rvizMarker->SetShape("cube");
        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.5, 0.0, 0.25-0.05/2.0)), "base_link");
        ros::Duration(0.5).sleep();
        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.5, 0.0, 0.25-0.05/2.0)), "base_link");

        // legs
        rvizMarker->SetShape("cylinder");
        rvizMarker->Scale(0.05, 0.05, 0.25);
        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.75-0.05, -0.5+0.05, 0.125)), "base_link");
        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.75-0.05, -0.5+0.05, 0.125)), "base_link");
        ros::Duration(0.5).sleep();
        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.25+0.05, -0.5+0.05, 0.125)), "base_link");
        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.25+0.05, -0.5+0.05, 0.125)), "base_link");
         ros::Duration(0.5).sleep();
 ...
 //        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.2,0.25, 0.45)), "base_link");
//        rvizMarker->Send(tf::Pose(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.4,0.25, 0.55)), "base_link");
        ...
        CObstacleDemo demo(nh, updater);

        // Enter loop to compute collision based on joint feedback
