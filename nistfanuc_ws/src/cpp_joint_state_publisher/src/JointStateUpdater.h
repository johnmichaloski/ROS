...
struct JointStateUpdater {
    ...
    void Update(std::vector<std::string> names, std::vector<double> values){
        ...
        std::cout << "Joint names " << RCS::VectorDump<std::string>(jointcmd.name).c_str() << "\n" << std::flush;
        std::cout << "Joint values " << RCS::VectorDump<double>(jointcmd.position).c_str() << "\n" << std::flush;
    ...
    virtual void SetJointNames(std::vector<std::string> jn) {
         joint_names=jn;
