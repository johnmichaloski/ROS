...
namespace Conversion {
    ...
    inline tf::Pose CreateRPYPose(std::vector<double> ds) {
        assert(ds.size() > 5);
        tf::Quaternion q;
        q.setRPY(ds[3], ds[4], ds[5]);
        return tf::Pose(q, tf::Vector3(ds[0], ds[1], ds[2]));
    }
    ...
    inline tf::Quaternion RPYDegrees(double roll, double pitch, double yaw) {
        tf::Quaternion q;
        q.setRPY(Deg2Rad(yaw), Deg2Rad(pitch), Deg2Rad(roll));\
        return q;
    }
    ...
    /*!
     * \brief Convert geometry_msgs::Quaterniont into an  tf quaternion.
     * \param point geometry_msgs::Quaterniont is rotation.
     * \return  tf::Quaternion.
     */
    template<>
    inline tf::Quaternion Convert<geometry_msgs::Quaternion, tf::Quaternion>(geometry_msgs::Quaternion gq) {
    ...
    template<>
    inline tf::Vector3 Convert<geometry_msgs::Point, tf::Vector3> ( geometry_msgs::Point  pt) {
