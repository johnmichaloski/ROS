...
int main(int argc, char** argv) {
	...
	crcl_pub = nh.advertise<nistcrcl::CrclCommandMsg>("crcl_command", 10);
	crcl_sub = nh.subscribe("crcl_status", 1000, crclstatusCallback);
	    ...
	    if( CAsioCrclSession::InMessages().SizeMsgQueue() > 0)
		{
                ...
                CAsioCrclSession* pSession = boost::get<1>(crclmsg);
                //_pSession = boost::get<1>(crclmsg);
#if 0
 		std_msgs::String msg;
		msg.data =crclcmd;
 		ROS_INFO("Crcl command: [%s]", msg.data.c_str());
#endif
 		nistcrcl::CrclCommandMsg msg;
		msg.xml =crclcmd;
